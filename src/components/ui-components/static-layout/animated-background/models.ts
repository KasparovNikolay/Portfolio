...
export class Canvas {
  ...
  resizeCanvas() {
    ...
    this.grid.x =
      Math.round(this.width / (this.lineWidth + this.lineGutter)) + 1;
  ...
  addLines() {
    for (let x = 0; x < this.grid.x; x++) {
      const posX = this.lineWidth * x + this.lineGutter * x;
      for (let y = 0; y < this.grid.y; y++) {
        const posY = this.lineWidth * y + this.lineGutter * y;
...
class Line {
  ...
  draw() {
    ...
    this.ctx.lineWidth = Math.pow(
      this.getDistance() / Math.abs(Math.sin(this.sine / 200) * 300),
      3.5
    );
